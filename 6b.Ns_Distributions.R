...
reps1 <- as.data.frame(reps[seq(1, nrow(reps), 5),])
reps1 <- reps1 %>% 
  mutate(sites=seq(1, nrow(reps), 5)) %>% 
...
##BMR
#calculate Ns
Ns_tmp <- Ns_calc(subset(all_families,all_families$loc == "bmrBL15"))
#isolating data to plot
df <- data.frame(sites=Ns_tmp[[1]]$sites,richness=Ns_tmp[[1]]$richness,sd=Ns_tmp[[1]]$sd,stringsAsFactors = F)
reps <- as.data.frame(Ns_tmp[[1]]$perm)
...
#calculate Ns
Ns_tmp <- Ns_calc(subset(all_families,all_families$loc == "bmrBL16"))
#isolating data to plot
df <- data.frame(sites=Ns_tmp[[1]]$sites,richness=Ns_tmp[[1]]$richness,sd=Ns_tmp[[1]]$sd,stringsAsFactors = F)
reps <- as.data.frame(Ns_tmp[[1]]$perm)
...
bmral_plot <- ggplot(df,aes(x=sites,y=richness))+
  ...
  xlab("Number of offspring sampled")+
  ylab("Number of parent genotypes")+
  ggtitle("C) Black Mallard River - Above Lake")
